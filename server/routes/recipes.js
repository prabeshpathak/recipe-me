...
async function populateRecipes(recipes, userId) {
  ...
  if (userId) currentUser = await User.findById(userId);
  ...
  await Ingredient.populate(recipes, {
    path: "ingredients",
    model: "Ingredient",
    select: "name",
  });
  ...
  if (currentUser) {
    recipes.forEach(async function (recipe) {
      recipe.set("isLiked", currentUser.favorites.includes(recipe._id), {
        strict: false,
      });
    });
  }
  ...
  router.get(constructPath(endpointPath, "/"), async function (req, res) {
    ...
    query.exec(async function (err, recipes) {
      ...
      res.json({
        ...
        filteredRecords: recipes.length,
        recipes: recipes,
      });
    });
  });
...
function authenticatedActions(router) {
  ...
  router.post(constructPath(endpointPath, "/"), async function (req, res) {
    ...
    if (!req.body || !req.body.name || !req.body.directions) {
      ...
      res
        .status(422)
        .json({ error: "Missing required recipe properties in request" });
      return;
    }
    ...
    recipe
      ...
      .catch(function (err) {
        res
          .status(422)
          .json({
            error: "Failed to create a recipe with provided properties",
          });
      });
  });
  ...
  router.post(
    constructPath(endpointPath, "/:id/favorite"),
    async function (req, res) {
      ...
      Recipe.findById(
        ...
        async function (err, recipe) {
          ...
          User.find(
            { _id: userId, favorites: req.params.id },
            function (err, user) {
              if (err) {
                res.status(422).json({ error: "Failed to execute query" });
                return;
    ...
    Recipe.findById(req.params.id, async function (err, recipe) {
      if (!recipe) {
        ...
        res
          ...
          .json({
            error:
              "The currently logged in user is not authorized to modify this recipe",
          });
      } else {
        Recipe.findByIdAndUpdate(
          req.params.id,
